...
class OracleRepository {
    ...
    async initPool() {
        ...
        try {
            this.pool = await oracledb_1.default.createPool({
                user: database_1.oracleConfig.user,
                password: database_1.oracleConfig.password,
                connectString: database_1.oracleConfig.connectString,
                poolMin: database_1.oracleConfig.poolMin,
                poolMax: database_1.oracleConfig.poolMax,
                poolIncrement: database_1.oracleConfig.poolIncrement
            });
    ...
    async updateSendFlag(xh) {
        let conn = null;
        try {
            conn = await this.getConnection();
            await conn.execute(`UPDATE jy_weijiz
    ...
    async updateHandleResult(xh, handleResult) {
        let conn = null;
        try {
            conn = await this.getConnection();
            await conn.execute(`UPDATE jy_weijiz
        SET ZT = :handleResult
        WHERE XH = :xh`, { handleResult, xh });
            await conn.commit();
        }
        finally {
            if (conn)
    ...
    async getDepartmentDoctors(departmentId) {
        ...
        try {
            conn = await this.getConnection();
            const result = await conn.execute(`SELECT DOCTOR_ID, DOCTOR_NAME, WECHAT_USER_ID
        FROM gy_zhigongxx
